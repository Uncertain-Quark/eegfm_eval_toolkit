# Code used from BIOT : https://github.com/ycq091044/BIOT/blob/main/model/sparcnet.py
...
class SPaRCNet(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        sample_length = int(n_seconds_input*sampling_rate) if sampling_rate is not None and n_seconds_input is not None else sample_length
    ...
    def forward(self, x, labels=None, return_embeddings: bool=False, **kwargs):
        ...
        return {
            "loss": loss,
            "logits": logits,
            "embeddings": emb if return_embeddings else None
        }
...
if __name__ == "__main__":
    sample_length = 750
    n_channels = 22
    X = torch.randn(2, n_channels, sample_length)
    model = SPaRCNet(n_channels=n_channels, sample_length=sample_length)
    out = model(X)
